...
type Player struct {
	...
	SpawnedThisRound  bool            //Set true on first spawn of a round; reset on StartMatch. Used to distinguish a real round-start spawn (flag=0) from a mid-match late-joiner (flag=1 → patched DLL force-dies them).
