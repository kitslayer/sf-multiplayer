...
var (
	...
	//fall back to relay-mode behavior for that lobby.
	loadedMaps = make(map[int32]*MapDataForLevel)
)

//MapDataForLevel is the server-side view of a single dumped scene. It's a slim
//projection of physics.MapData (we don't import physics here to keep levels.go
//focused on the wire-level Level type) but the JSON shape is identical so we
//can fill it from the same JSON files.
type MapDataForLevel struct {
	SceneIndex      int32          `json:"sceneIndex"`
	Name            string         `json:"name"`
	StaticColliders []ColliderJSON `json:"staticColliders"`
	PlayerSpawns    []SpawnJSON    `json:"playerSpawns"`
	WeaponSpawns    []SpawnJSON    `json:"weaponSpawns"`
	Killboxes       []KillboxJSON  `json:"killboxes"`
}

//ColliderJSON / SpawnJSON / KillboxJSON mirror the dumper's JSON schema.
...
type KillboxJSON struct {
	Pos  [3]float32 `json:"pos"`
	Size [3]float32 `json:"size"`
}

//SceneIndex returns the Landfall scene index for a Level (0 if not Landfall).
//Exposed as a method because the field is unexported.
func (m *Level) SceneIndex() int32 { return m.sceneIndex }

//MapData returns the dumped map JSON for this Level (M1+), or nil if no dump
//exists for the level's scene index.
func (m *Level) MapData() *MapDataForLevel { return m.mapData }

//BindMapData attaches dumped JSON data to the Level. Idempotent.
func (m *Level) BindMapData(d *MapDataForLevel) { m.mapData = d }
...
type Level struct {
	...
	//Phase 5 server-authoritative state (M1+). Populated by Lobby.bindMapData
	//when the dumped JSON for this level's sceneIndex exists; nil otherwise (we
	//fall back to relay-mode weapon spawn behavior for un-dumped levels).
	mapData *MapDataForLevel
