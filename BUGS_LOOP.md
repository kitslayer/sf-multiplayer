...
| LOBBY-1 | lobby | WIP | Lobby-switching backend. **Baseline: `sf-router` `go test ./...` is GREEN** (router/select/registry all pass, 1.18s). The *tested* selection/registry logic is sound, so the "switching broken" symptom is either an UNtested router path or the UI layer (→ LOBBY-2 / `serve-lobbies.py`). Next: read `select.go`/`registry.go` for untested switching transitions; Playwright-screenshot `serve-lobbies.py` (**B**); reproduce kit's exact symptom as a failing test before any fix. |
