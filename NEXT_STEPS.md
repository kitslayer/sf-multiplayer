...
>
> **Latest (2026-06-23): autonomous polish loop merged to `main` (NOT deployed; versions unchanged).** A 15-iteration hunt across lobby/boxes/anti-cheat — full log + evidence in [`BUGS_LOOP.md`](BUGS_LOOP.md) + `loop-evidence/`, summary in [`WHATS_NEW.md`](WHATS_NEW.md). Shipped 4 low-risk fixes (web-browser favicon 204; mid-match lobby-switch feedback banner #5; two defense-in-depth bounds in the host damage-handler + snapshot serializer) + a pinned `Skip`'d router regression test. Most of the seeded backlog verified **code-correct / by-design** (P0-23 boxes-to-void NOT reproducing on a live headless oracle; P0-24 round-advance re-arm intact; AC-1 rate-guard observe-only as designed). **Kit's remaining punch list (needs a real 2-player session — see [`BUGS_LOOP.md`](BUGS_LOOP.md) → "Punch list for kit"): confirm void/lava death (OPEN-1/2), confirm the lobby-switch symptom isn't just the documented per-IP limit (LOBBY-1), confirm rigs respawn in rounds 2+ (P0-24).** Then a `.115` deploy + version bump if accepted.
>
