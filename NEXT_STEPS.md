...
> **Current state — this file + [`README.md`](README.md) are the source of truth; the other status docs are historical snapshots.** Live server: **`69.53.117.43`** (game UDP 1337, router UDP 1338, lobby browser TCP 8080). Maintainer: kit (solo; ALKA departed, access fully revoked). **Server-side deployed on `.115` (2026-07-01):** `SFHeadlessHost 0.4.5` + `SFBoxFix 0.3.3` — verified live (v25 Ping OK on LAN + public `69.53.117.43:1337`; oracle heartbeating; MAIN lobby registered + alive in the browser). The two `NullReferenceException`s in the boot log are pre-existing game-code headless-boot noise (`WeaponPickUp`/`NetworkSyncableObject` `.Awake` under `MultiplayerManager:Awake`), unrelated to our plugins. **Client-side installer refreshed (2026-07-01):** `SFClientRecon 0.6.5` + `SFServerBrowser 0.5.8` shipped in `sf-multiplayer-StickFight-Installer.zip` + `dist/` + `1-click-install/files/`. The in-game **server browser is complete, with no website**: client repointed to the sf-router single port (`1338`, was MAIN-direct `1337`) so BROWSE/JOIN/switch route through it; browser fetches the lobby LIST + does CREATE/RESTART over the router's UDP port (`LIST`/`CREATE`/`RESTART` ops, not HTTP `:8080`); duplicate uGUI overlay disabled (one IMGUI UI, F2). **Always-on lobbies:** MAIN/COMP/CASUAL are systemd-managed (`sf-oracle` + `sf-lobby@COMP`/`@CASUAL`, `static=true` → reaper-exempt, reboot-persistent). Router (`-default MAIN` + LIST + CREATE/RESTART proxy w/ token) + serve-lobbies (`/lobbies/restart` + `SF_MAX_LOBBIES=8`) + sudoers all live on `.115` (verified: RESTART bounced COMP, CREATE spawned a lobby). **Only open item: a visual GUI eyeball on a real client** (functionally verified; automated Xvfb screenshot was too slow — 30 s check for you: install → F2 → BROWSE). **Still owed:** a 2-player live *disconnect* re-verify of the host orphan-rig/`Finite` fixes (the deploy passed boot + Ping but not a full match), and the flagged PowerShell installer-script hardening below (needs a Windows test — deferred, not shipped).
>
> **Latest (2026-07-02): always-on lobbies + in-game create/restart (no website).** COMP/CASUAL are now systemd-managed (templated `sf-lobby@.service`, reuses `start-oracle-server.sh` + `register-main-lobby.sh` → `static=true` reaper-exempt, reboot-persistent), like MAIN. Added router CREATE (`0x04`) / RESTART (`0x05`) control ops that proxy to the local control plane (token held server-side): `serve-lobbies` gained `POST /lobbies/restart` (bounces the named lobby's systemd unit via a scoped NOPASSWD sudoers rule); `SFServerBrowser` 0.5.8 does CREATE over UDP + a per-row `RST` button. Verified live: RESTART bounced COMP's unit; CREATE spawned a lobby through the router. Bumped `SF_MAX_LOBBIES` 6→8 for free CREATE ports (1343+). Router + serve-lobbies redeployed to `.115`; sudoers installed. **Owed: a visual GUI eyeball on a real Proton client** (the automated Xvfb screenshot was too slow; everything is functionally verified). Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-07-01, list-over-UDP): lobby list now comes over the router's UDP port — no website.** Per "just simple in-game browsing, no separate website": added a `LIST` control op to sf-router (`0x03`; replies with live code + players + capacity from its registry) and switched `SFServerBrowser` (0.5.7) to fetch the list via UDP (`FetchServersUdpCoroutine`) instead of HTTP `:8080`. Deployed + verified live (LIST → MAIN+COMP+CASUAL). Removes the TCP-8080 dependency for *browsing*; CREATE still uses HTTP (moving create/restart onto the UDP control channel — spawn/restart lobbies in-game — is next, tracked). `go test -race` green; shipped browser 0.5.7 in installer + dist + 1-click. Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-07-01, even later): in-game SERVER BROWSER fixed end-to-end — router wiring + GUI + multi-lobby.** The router "never worked" because the shipped client connects to `-port 1337` (MAIN-direct), bypassing the sf-router on `1338` — so its lobby-SELECTs hit the MAIN oracle, which ignores them. Repointed the client (`SfOracleLobbyConnect` default) + installer/launch scripts to **`1338`**; the router's new `-default MAIN` (deployed + verified live) catches no-SELECT traffic. The GUI was "all fucked up" because `SFServerBrowser` instantiated **two** overlapping overlays (IMGUI + a uGUI `LobbyOverlay` that also spun up its own `EventSystem`) — disabled the uGUI one, kept the polished IMGUI browser, bound **F2** to it. Stood up `COMP` (1341) + `CASUAL` (1342) backends on `.115`; verified live: `SELECT COMP` → ACK OK, router `byCode {"COMP":2}`, list shows MAIN+COMP+CASUAL. **Shipped** client `0.6.5` + browser `0.5.6` in the installer + `dist/` + `1-click/` (sha-identical). **Owed:** public LIST transport (the TCP-8080 home-router forward is gone → re-add it, or serve the list over the router's UDP port; *joining* is already public via 1338); always-on lobby persistence (COMP/CASUAL are ephemeral `launch-lobby.sh` backends → systemd + reaper-exempt like MAIN); and an in-game visual GUI check on a real (Proton) client. Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-07-01, later): client NaN/+Inf "vanish" guard for network-synced objects (on `main`, not deployed).** Added [`sf-client-recon/SfNsoNaNGuard.cs`](sf-client-recon/SfNsoNaNGuard.cs) — a defensive `NetworkSyncableObject.SyncObjectState` guard: prefix back-dates a same-frame packet burst to dodge the `distance / Δt` +Inf (`:431`/`:433`); postfix fixes a degenerate zero up-vector and rescues an already-NaN transform to `m_EndPos`. Fixes the client-side "boxes/platforms/barrels/lasers vanish on a laggy link → NaN crash" bug. **Prompted by [z7572/NaNFixer](https://github.com/z7572/NaNFixer)** (credited in-source; independently reimplemented, not copied). Client-only — the host forces `mHasControl=true`, gating `LerpLocalDummy` off — and it fills a gap our inbound validators miss (they reject bad *data*; this stops the game manufacturing +Inf from *valid* packets). **Shipped:** bumped to **0.6.4**, rebuilt clean, propagated into `dist/` + `1-click-install/files/` + the installer zip payload (all four copies sha256-identical), and static-verified the shipped DLL (version + guard methods + all six reflection field-name literals match the decompile). `SF_NANGUARD=0` disables. Also refined the ~24h crash lead in [`notes/CRASH_INVESTIGATION.md`](notes/CRASH_INVESTIGATION.md). **Owed (one runtime confirmation): an in-game `[nan-guard] active` load-check + a real burst repro** — not launched here (guard fails safe, so players are never worse off). Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-07-01): deployed host `0.4.5` to `.115` + refreshed the player installer to client `0.6.3` / browser `0.5.5`.** After merging the repo simplification (partial-class split) and applying PR #14's 30→60 Hz doc fix, bumped versions, rebuilt all four plugins clean (net46/net35, 0 warnings), and shipped: backed up the live `0.4.3` DLL on-box (`SFHeadlessHost.dll.bak-20260701-211445`), swapped in `0.4.5`, restarted `sf-oracle.service`, verified a clean boot (0.4.5 banner, port 1337 bound, heartbeat) + v25 Ping on LAN and public `69.53.117.43`. Rebuilt the installer zip's `StickFight-DropIn` payload with the current client DLLs (BepInEx core + patched `Assembly-CSharp.srv.v25` unchanged) and refreshed `dist/`. SFBoxFix unchanged (byte-identical `0.3.3`, not redeployed). **Deferred (needs a Windows test):** the PowerShell installer clobber/rollback/wrong-DLL fixes. Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-06-30): third review sweep — six reviewers; control-plane + host + client fixes on `main`.** Six parallel component reviewers hunting *new* correctness bugs. Shipped: a static-lobby guard on the HTTP stop path (`POST /lobbies/stop` → `stop-lobby.sh` had no `static=true` guard, so an `SF_CONTROL_TOKEN` holder could stop MAIN and `rm -rf` its wineprefix — now refused authoritatively in `stop-lobby.sh` + a `409` in `_handle_stop`); `_handle_stop` body-read hardening; **host** orphaned-rig-on-disconnect cleanup + `Finite()` guard on the NSO rotation/up fields (`SFHeadlessHost.cs`); **client** out-of-order-snapshot drop (with server-restart resync) + dead-`FindObjectsOfType` removal; **server-browser** F3-HUD roster throttled to 2 Hz; plus low-risk infra/client hardening. Router + box-fix came back clean. All compile/test-verified (`dotnet build` clean, 15 Python tests green, `go test -race` green). **Versions unchanged — bump at deploy/installer-refresh time;** the host changes touch the live sim and need a 2-player live re-verify before the `.115` deploy. Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-06-26, later): second multi-agent review sweep — ops/infra + client + host fixes on `main`.** Targeted fresh surface (the live-server shell scripts + the player installer) plus 2nd host/client passes. Shipped: control plane can no longer reap the systemd MAIN oracle (it'd kill it + `rm -rf` its wineprefix — `static=true` guard in the reaper + `stop-all-lobbies.sh`); `launch-lobby.sh` port-leak on crashed lobbies; client lobby-switch slot reset + cache hygiene (`_recentLerpAt`, mapSync refresh); host clears in-flight projectiles at round end (`0.4.3`→`0.4.4`). **Flagged for the installer refresh (PowerShell — needs a Windows test, not edited blind):** the shipped zip's `install.ps1` clobbers a player's pre-existing BepInEx loader/config with no backup and uninstall doesn't restore it (breaks the "restores exactly as it was" promise); uninstaller backup-deletion parity; no partial-failure rollback; the deprecated `dist/`/`deploy/` client installers ship the wrong DLL. Full log in [`WHATS_NEW.md`](WHATS_NEW.md).
>
> **Latest (2026-06-26): deployed `SFHeadlessHost 0.4.3` + `SFBoxFix 0.3.3` to `.115`.** First `.115` deploy since the 2026-06-11 keeper — it carries everything merged since (the 2026-06-17 review's server-auth-rig-respawn + audit fixes, the polish loop, and the 2026-06-25 sweep's host NaN-sanitization + box instance-id-cache + void-budget/reconnect/empty-map guards). Backed up the keeper DLLs on-box (`*.bak-20260626-0017`); restarted `sf-oracle.service`; verified the `0.4.3`/`0.3.3` boot banners, a fresh oracle bound on 1337, and a clean v25 Ping reply on both LAN and `69.53.117.43`. The `mHasControl` HIGH item is still flagged for a live 2-player session (its code is unchanged, so the deploy doesn't affect it either way).
>
> **Latest (2026-06-25): multi-agent code-review sweep — 9 fixes on `main`, pushed (NOT deployed; versions unchanged).** Four parallel reviewers across host/client/browser/box/router/infra, triaged + fixed by hand. Landed: non-finite snapshot hardening (host+client — the live +Inf-class bug), SFBoxFix instance-id cache correctness, host void-budget/reconnect-input/empty-map-pool guards, sf-monitor `?n=` hardening + tooling. **Flagged for a live session before any code change:** the `Controller.mHasControl` gate — the client fire packet (msgType 41) and local ice/destructible breaks may be inert in oracle mode (confirm `mHasControl` is really always-false live first). Full log in [`WHATS_NEW.md`](WHATS_NEW.md). C# changes need a 2-player live re-verify before deploy.
>
