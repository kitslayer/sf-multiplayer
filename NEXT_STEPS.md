...
> **Current state — this file + [`README.md`](README.md) are the source of truth; the other status docs are historical snapshots.** Live server: **`69.53.117.43`** (game UDP 1337, router UDP 1338, lobby browser TCP 8080). Maintainer: kit (solo; ALKA departed, access fully revoked). **Server-side deployed on `.115` (2026-06-26):** `SFHeadlessHost 0.4.3` + `SFBoxFix 0.3.3` — verified live (v25 Ping OK on LAN + public; 0 boot exceptions). These carry the 2026-06-17 review + 2026-06-22 polish loop + 2026-06-25 sweep server-side fixes. **Client-side** (`SFClientRecon`, `SFServerBrowser`) reaches players only through the **installer**, which has NOT been refreshed — players still run the keeper `0.6.1`/`0.5.3`, so the sweep's client-side NaN guard isn't shipped yet (it's defense-in-depth; the host already prevents emitting non-finite). Next client release: bump `0.6.3`/`0.5.5` + rebuild the installer zip.
>
