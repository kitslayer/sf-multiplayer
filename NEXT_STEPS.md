...
> **Current state — this file + [`README.md`](README.md) are the source of truth; the other status docs are historical snapshots.** Live server: **`69.53.117.43`** (game UDP 1337, router UDP 1338, lobby browser TCP 8080). Maintainer: kit (solo; ALKA departed, access fully revoked). **Server-side deployed on `.115` (2026-06-26):** `SFHeadlessHost 0.4.3` + `SFBoxFix 0.3.3` — verified live (v25 Ping OK on LAN + public; 0 boot exceptions). **`main` has since moved to host `0.4.4`** (the 2026-06-26 second sweep's round-boundary projectile-clear; undeployed — `.115` stays `0.4.3` until a redeploy). The infra-script fixes from that sweep (reaper/stop-all-lobbies static-lobby guard, etc.) also take effect on `.115` only on the next rsync + service restart. **Client-side** (`SFClientRecon`, `SFServerBrowser`) reaches players only through the **installer**, which has NOT been refreshed — players still run the keeper `0.6.1`/`0.5.3`, so the client-side fixes (the sweep NaN guard + the 2nd-sweep lobby-switch/cache fixes) aren't shipped yet. Next client release: bump `0.6.3`/`0.5.5`, **apply the flagged installer fixes (below) with a Windows test**, rebuild the installer zip.
>
